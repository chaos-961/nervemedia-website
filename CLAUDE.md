...
# A preview config also exists at .claude/launch.json (name: "nerve-static", port 4173).
...
**The deploy is a filtered copy, not the whole repo.** The workflow assembles `_site` from only `index.html`, `styles.css`, `app.js`, `assets/`, and `admin/`. Dev-only files (`scripts/`, `README.md`, `CLAUDE.md`, `ToDoList.txt`, `.claude/`) stay in the repo but never reach Pages. **When you add a new top-level website file or folder, add it to the `Assemble site` step** or it won't be served.
